/**
 * AiThinkWise - Main JavaScript File
...
function initializeContactForm() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            // Get and sanitize form data (matching EmailJS template field names)
            const formData = {
                first_name: sanitizeInput(document.getElementById('encoreFirstName').value),
                last_name: sanitizeInput(document.getElementById('encoreLastName').value),
                email: sanitizeInput(document.getElementById('encoreEmail').value),
                phone: sanitizeInput(document.getElementById('encorePhone').value),
                message: sanitizeInput(document.getElementById('encoreMessage').value),
            ...
            if (country && country.value) {
                formData.country = sanitizeInput(country.value);
            } else {
                formData.country = 'Not specified';
            }
            
            if (service && service.value) {
                formData.service = sanitizeInput(service.value);
            } else {
...
function validateForm() {
    ...
    fields.forEach(field => {
        ...
        if (!value) {
            ...
            if (field.element.id === 'encoreEmail' && !isValidEmail(value)) {
                field.element.classList.add('encore-form-error');
                missingFields.push('Valid Email Address');
                isValid = false;
            } else if (field.element.id === 'encorePhone' && !isValidPhone(value)) {
                field.element.classList.add('encore-form-error');
...
function showAlert(message, type = 'error') {
    ...
    safeSetHTML(alert, `
        ...
        <span class="encore-alert-message">${sanitizeInput(message)}</span>
    `);
...
function initializeChatWidget() {
    ...
    function showTypingIndicator() {
        ...
        safeSetHTML(typingDiv, `
            <div class="encore-chat-avatar">
                <img src="Files/AiThinkWise_2.PNG" alt="AiThinkWise AI" class="encore-chat-avatar-img">
            </div>
...
function initializeSearch() {
    ...
    const searchData = [
        ...
        },
        {
            ...
            description: "AiThinkWise is a growing provider of professional digital services, specializing in website development and automation solutions.",
            type: "About",
            url: "index.html",
            keywords: ["professional", "digital", "services", "website", "development", "automation"]
        },
        {
            title: "Why Choose AiThinkWise",
